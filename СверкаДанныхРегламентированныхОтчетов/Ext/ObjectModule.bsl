Функция СформироватьОтчет(Организация, Период) Экспорт
	...
	ДополнитьПоказатели(Организация, Показатели, НачалоГода(Период), КонецДня(Период));
...
Процедура ДополнитьПоказатели(Организация, Показатели, НачалоПериода, ОкончаниеПериода)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	НДСРеализация0Остатки.НДСОстаток КАК Сумма
	|ИЗ
	...
	Если Выборка.Следующий() Тогда
		НДСКПодтверждению = Выборка.Сумма;
	КонецЕсли;
КонецПроцедуры
...
Процедура ВставитьПоказатели_НДС(ТаблицаПоказателей, ОписаниеОтчета)
	...
	// 10%
	ЗначениеПоказателя = ПоказательОтчета(Раздел4, "П000400001003");
	НоваяСтрока = ТаблицаПоказателей.Добавить();
	ЗаполнитьЗначенияСвойств(НоваяСтрока, ОписаниеОтчета);
