Функция СформироватьОтчет(Организация, Период) Экспорт
	...
	ДополнитьПоказателиУчетнымиДанными(Организация, Показатели, Периоды);
	...
	ТабличныйДокумент.Вывести(Область);
	ВывестиЗначенияПоказателей(Макет, Периоды, "РазрешенныеРазницыИтог", ТаблицаСверкиДанных, ТабличныйДокумент);
	...
	ТабличныйДокумент.Вывести(Область);
	ВывестиЗначенияПоказателей(Макет, Периоды, "ВозвратПоставщику", ТаблицаСверкиДанных, ТабличныйДокумент);
	
	Область = Макет.ПолучитьОбласть("СтрокаСреднийУровень|Показатель");
	Область.Параметры.Показатель = "Возвраты товаров от покупателей";
	ТабличныйДокумент.Вывести(Область);
	ВывестиЗначенияПоказателей(Макет, Периоды, "ВозвратОтПокупателя", ТаблицаСверкиДанных, ТабличныйДокумент);
...
Функция ТаблицаСверкиДанныхРегламентированныхОтчетов(Показатели, Периоды)
	...
	// Разрешенные разницы
	ДобавитьПоказательНакопительный(ТаблицаСверкиДанных, "РазрешенныеРазницы", "НеподтвержденныйНДС", Периоды, Показатели);
...
Процедура ДобавитьПоказательОборотный(ТаблицаСверкиДанных, ИсточникДанных, ИмяПоказателя, Периоды, Показатели, Итоговый = Ложь)
...
Процедура ДобавитьПоказательРазница(ТаблицаСверкиДанных)
	...
	Для Каждого Колонка Из ТаблицаСверкиДанных.Колонки Цикл
		...
		Если МассивКолонокИсключений.Найти(ИмяКолонки) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		СтрокаТаблицы[ИмяКолонки] = СтрокаПрибыль[ИмяКолонки] - СтрокаНДС[ИмяКолонки] - СтрокаРазрешенныеРазницы[ИмяКолонки];
	КонецЦикла;
...
Процедура ВставитьПоказатели_Прибыль(ТаблицаПоказателей, ОписаниеОтчета)
	...
	Если ОписаниеОтчета.ВыбраннаяФорма = "ФормаОтчета2016Кв4"
		...
		НоваяСтрока.ЗначениеПоказателя = Окр(ЗначениеПоказателя, 0);
...
Процедура ДополнитьПоказателиУчетнымиДанными(Организация, Показатели, Периоды)
...
Процедура ДополнитьПоказателиУчетнымиДаннымиВозвратПоставщику(Организация, Показатели, СтрокаПериода)
	...
	Если Выборка.Следующий() Тогда
		СтрокаПоказателя.ЗначениеПоказателя = Окр(Выборка.Сумма, 0);
	КонецЕсли;
