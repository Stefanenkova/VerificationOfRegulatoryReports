Функция СформироватьОтчет(Организация, Период) Экспорт
	...
	//"Доходы от реализации" + Символы.ПС + "Лист 02, строка 010";
	// База прибыль
	...
	ТабличныйДокумент.Вывести(Область);
	ВывестиЗначенияПоказателей(Макет, Периоды, "ДоходыОтРеализации", ТаблицаСверкиДанных, ТабличныйДокумент);
	
	// База прибыль
	Область = Макет.ПолучитьОбласть("СтрокаНижнийУровень|Показатель");
	Область.Параметры.Показатель = "Внереализационные доходы" + Символы.ПС + "Лист 02, строка 020";
	ТабличныйДокумент.Вывести(Область);
	ВывестиЗначенияПоказателей(Макет, Периоды, "ВнереализационныеДоходы", ТаблицаСверкиДанных, ТабличныйДокумент);
	...
	// База НДС
	Область = Макет.ПолучитьОбласть("СтрокаСреднийУровень|Показатель");
	Область.Параметры.Показатель = "Налоговая база по НДС" + Символы.ПС + "Раздел 3, Строка 010";
	ТабличныйДокумент.Вывести(Область);
	...
	ТабличныйДокумент.Вывести(Область);
	ВывестиЗначенияПоказателей(Макет, Периоды, "Выручка18", ТаблицаСверкиДанных, ТабличныйДокумент);

	Область = Макет.ПолучитьОбласть("СтрокаНижнийУровень|Показатель");
	Область.Параметры.Показатель = "НДС по ставке 10%" + Символы.ПС + "Раздел 3, строка 020";
	ТабличныйДокумент.Вывести(Область);
	ВывестиЗначенияПоказателей(Макет, Периоды, "Выручка10", ТаблицаСверкиДанных, ТабличныйДокумент);
	
	// Разница
	Область = Макет.ПолучитьОбласть("СтрокаВерхнийУровень|Показатель");
	Область.Параметры.Показатель = "Разница";
	ТабличныйДокумент.Вывести(Область);
	ВывестиЗначенияПоказателей(Макет, Периоды, "Разница", ТаблицаСверкиДанных, ТабличныйДокумент);

	Возврат ТабличныйДокумент;
	
КонецФункции

Процедура ВывестиЗначенияПоказателей(Макет, Периоды, Показатель, ТаблицаСверкиДанных, ТабличныйДокумент)
...
Функция ТаблицаСверкиДанныхРегламентированныхОтчетов(Показатели, Периоды)
	...
	СтрокаТаблицы = ТаблицаСверкиДанных.Добавить();
	СтрокаТаблицы.ИмяПоказателя = "ДоходыОтРеализации";
	МассивПериодовДляРасчета = Новый Массив;
	Для Каждого СтрокаПериода Из Периоды Цикл
		...
		Отбор = Новый Структура("ДатаОкончания, ИсточникОтчета, Показатель", СтрокаПериода.ДатаОтчета, "РегламентированныйОтчетПрибыль", "СуммаДоходовОтРеализации");
		СтрокиПоказателей = Показатели.НайтиСтроки(Отбор);
		Для Каждого СтрокаПоказателя ИЗ СтрокиПоказателей Цикл
	...
	СтрокаТаблицы = ТаблицаСверкиДанных.Добавить();
	СтрокаТаблицы.ИмяПоказателя = "ВнереализационныеДоходы";
	МассивПериодовДляРасчета = Новый Массив;
	Для Каждого СтрокаПериода Из Периоды Цикл
		...
		Отбор = Новый Структура("ДатаОкончания, ИсточникОтчета, Показатель", СтрокаПериода.ДатаОтчета, "РегламентированныйОтчетПрибыль", "ВнереализационныеДоходы");
		СтрокиПоказателей = Показатели.НайтиСтроки(Отбор);
		Для Каждого СтрокаПоказателя ИЗ СтрокиПоказателей Цикл
			СтрокаТаблицы[СтрокаПериода.ИмяКолонки] = СтрокаТаблицы[СтрокаПериода.ИмяКолонки] + СтрокаПоказателя.ЗначениеПоказателя;
		КонецЦикла;
	...
	// Расставим строки
	// 7. Декларация по НДС - База НДС
	СтрокаТаблицы = ТаблицаСверкиДанных.Добавить();
	СтрокаТаблицы.ИмяПоказателя = "Выручка18";
	МассивПериодовДляРасчета = Новый Массив;
	Для Каждого СтрокаПериода Из Периоды Цикл
		Если СтрокаПериода.Накопительный Тогда
			МассивПериодовДляРасчета.Добавить(СтрокаПериода.ИмяКолонки);
		...
		Отбор = Новый Структура("ДатаОкончания, ИсточникОтчета, Показатель", СтрокаПериода.ДатаОтчета, "РегламентированныйОтчетНДС", "Выручка18");
		СтрокиПоказателей = Показатели.НайтиСтроки(Отбор);
		Для Каждого СтрокаПоказателя ИЗ СтрокиПоказателей Цикл
			СтрокаТаблицы[СтрокаПериода.ИмяКолонки] = СтрокаТаблицы[СтрокаПериода.ИмяКолонки] + СтрокаПоказателя.ЗначениеПоказателя;
		КонецЦикла;
	...
	// Расставим строки
	// 8. Декларация по НДС - База НДС
	СтрокаТаблицы = ТаблицаСверкиДанных.Добавить();
	СтрокаТаблицы.ИмяПоказателя = "Выручка10";
	МассивПериодовДляРасчета = Новый Массив;
	Для Каждого СтрокаПериода Из Периоды Цикл
		Если СтрокаПериода.Накопительный Тогда
			МассивПериодовДляРасчета.Добавить(СтрокаПериода.ИмяКолонки);
		...
		Отбор = Новый Структура("ДатаОкончания, ИсточникОтчета, Показатель", СтрокаПериода.ДатаОтчета, "РегламентированныйОтчетНДС", "Выручка10");
		СтрокиПоказателей = Показатели.НайтиСтроки(Отбор);
		Для Каждого СтрокаПоказателя ИЗ СтрокиПоказателей Цикл
			СтрокаТаблицы[СтрокаПериода.ИмяКолонки] = СтрокаТаблицы[СтрокаПериода.ИмяКолонки] + СтрокаПоказателя.ЗначениеПоказателя;
		КонецЦикла;
	...
	Для Каждого СтрокаПериода Из Периоды Цикл
		СтрокаТаблицы[СтрокаПериода.ИмяКолонки] = 
			СтрокаТаблицыБазаПрибыль[СтрокаПериода.ИмяКолонки] - СтрокаТаблицыБазаНДС[СтрокаПериода.ИмяКолонки];
	КонецЦикла;
